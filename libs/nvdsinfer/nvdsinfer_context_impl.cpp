...
NvDsInferContextImpl::allocateBuffers()
{
    ...
    for (size_t iL = 0; iL < m_AllLayerInfo.size(); iL++)
    {
        ...
        size_t sizePerBatch = layerDims.numElements *
            getElementSize(layerInfo.dataType);
    ...
    if (maxSize > 0)
    {
        ...
        CHECK_CUDA_ERR_NO_ACTION(
            cudaMalloc(&m_inputDumpDeviceBuf, maxSize),
...
void dump_to_file(
        ...
        int toRawdata, NvDsInferFormat format)
    {
        if (toRawdata)
        {
        ...
        for (size_t i = 0; i < m_AllLayerInfo.size(); i++)
        {
            ...
            if (info.isInput)
            {
                ...
                for (int b = 0; b < batchSize; b++)
                {
                    ...
                    std::string filename =
                        std::string(SAVE_FOLDER) + std::to_string(m_UniqueID) +
